...
const HeroImageSelector: React.FC<HeroImageSelectorProps> = ({
  ...
  return (
    <section className="px-4 sm:px-6 lg:px-8 xl:px-15">
      <div className="relative w-full min-h-[400px] sm:min-h-[500px] md:min-h-[600px] lg:min-h-[750px] overflow-hidden">
        ...
        {/* Content Container */}
        <div className="relative  z-10 h-full flex flex-col justify-end items-end p-4 sm:p-6 md:p-8 lg:p-12">
          {/* Bottom: Left Content & Right Thumbnails */}
          <div className="flex justify-between items-end gap-4 sm:gap-6 md:gap-8 lg:gap-12 w-full">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-200 mb-4 sm:mb-6 md:mb-8 leading-relaxed line-clamp-2 sm:line-clamp-3">
                {selectedImage?.description}
              </p>
              <Button
                size="lg"
                className="w-full sm:w-auto bg-[#4A69E2] text-white font-semibold px-4 sm:px-6 md:px-8 py-3 sm:py-4 md:py-6 rounded-lg sm:rounded-xl tracking-widest text-xs sm:text-sm md:text-base whitespace-nowrap"
              >
                SHOP NOW
              </Button>
            </div>

            {/* Right: Image Thumbnails - Always Vertical */}
            <div className="flex  flex-col gap-2 sm:gap-3 md:gap-4 lg:gap-4 flex-shrink-0">
              {images.map((image) => (
                <button
                  key={image.id}
